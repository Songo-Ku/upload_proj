...
class ImageListCreateViewSetBasicPlanTestCase(APITestCase):
    ...
    def test_multiple_variants_with_post_image_basic(self):
        ...
        self.assertEquals('duration not defined', response.data.get("duration", "duration not defined"))
    ...
    def test_define_not_permited_fields_basic_tier(self):
        valid_image = {
            "image": self.simply_upload_test_file,
            "duration": 3000
        }
        response = self.client.post(
            ...
            format="multipart",)
        self.assertEquals(
            "no exist duration in response as field",
            response.data.get("duration", "no exist duration in response as field"))
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        self.assertEquals(response.data.get('thumbnail_400px', False), False)
